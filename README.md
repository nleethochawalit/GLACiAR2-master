...
These are the files generated by *SExtractor*, which are the catalouges in fits format and the segmentation maps:
   ...
   c. The segmentation maps indicating the source boundaries and indices. Unless indicated otherwise, they will be deleted in each iteration. To keep these maps, run *GLACiAR2* with an option -m False e.g. *python completeness.py -m False parameters.yaml*.
